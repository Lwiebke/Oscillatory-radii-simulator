//Sub rutina diseñada para ubicar N particulas con radios > 0 en espacio sin superposición con
...
vector <Particula> ubicar_particulas(double W, double L, double D,int N_deseado,float radio_base, float amplitud, float periodo,double v_deseada,double tau, double m){
  ...
  while (colisiones < tolerancia_col && i-2 < N_deseado){
    ...
    if (j == i){ //si recorrí todas las particulas previamente ubicadas sin colisiones, paso a la siguente
      ...
      aux.set_vel_x(0);
      aux.set_vel_y(0);
      aux.set_pos_x_a(posx);
      aux.set_pos_y_a(posy);
...
//encontrar nueva posicion
  while (!done) {
    ...
    if (j == particulas.size()){ //si recorrí todas las particulas previamente ubicadas sin colisiones, paso a la siguente
      ...
      particulas[id].set_pos_x_a(posx);
      particulas[id].set_pos_y_a(posy);
