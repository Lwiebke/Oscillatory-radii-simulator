...
class Particula{
  ...
  public:
    ...
    void set_pos_y_a(double ay) {pos_y_a = ay;}
    void set_pos_x_a(double ax) {pos_x_a = ax;}
    ...
    double get_pos_y_a()const {return pos_y_a;}
    double get_pos_x_a()const {return pos_x_a;}
    ...
    vector<double> get_fuerza_migratoria(){
      ...
      vector<double> fm;
      fm.push_back( (masa * (v_deseada*e_target[0] - vel_x))/tau);
      fm.push_back( (masa * (v_deseada*e_target[1] - vel_y))/tau);
      return (fm);
...
Particula::Particula (const Particula& a_copiar){
  ...
  pos_x_a = a_copiar.pos_x_a;
  pos_y_a = a_copiar.pos_y_a;
