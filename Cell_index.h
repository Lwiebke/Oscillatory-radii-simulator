...
void asignar_particulas_celdas(vector<vector<list<int> > > &grilla, vector<Particula> &particulas,float altura, float ancho){
  ...
  for (int id = 0; id < N; id++){//para cada particula
    int pertenencia_celda_x = M2-1;//si recorre todo el ciclo y nunca entra a los if, es porque se encuentra en la última celda posible,  "M-1".
...
bool son_vecinas(Particula *A, Particula *B){
  bool vecindad = false;
  float distancia;
  ...
  if (distancia <= (A->get_radio()+B->get_radio() ))
    vecindad = true;
...
vector<vector<int> > calcular_vecinos(vector<Particula> &particulas, float W, float L){
          ...
          for (auto primera = grilla[i][j].begin(); primera!=grilla[i][j].end(); ++primera){
            ...
            for (auto segunda =  grilla[i][j].begin(); segunda!=grilla[i][j].end(); ++segunda){
              if (id1 < *segunda){ //misma celda
                id2 = *segunda;
                if(son_vecinas(&particulas[id1], &particulas[id2])){
